...
#pragma omp parallel for
    for (int i = 0; i < triangles.size(); i++) {
        ...
        */
        ...
        Eigen::HouseholderQR<Eigen::Matrix<double, 3, 2> > quadraticDecomposition(triangles[i].getDeformationGradient());
        ...
        */
        /*
        double sqrtDiscrim = sqrt( (symmetricShapeOperator(0,0) - symmetricShapeOperator(1,1)) * (symmetricShapeOperator(0,0) - symmetricShapeOperator(1,1)) + 4.0 * symmetricShapeOperator(0,1) * symmetricShapeOperator(0,1) );
        double princCurv1 = 0.5 * (symmetricShapeOperator(0,0) + symmetricShapeOperator(1,1) + sqrtDiscrim);
        double princCurv2 = 0.5 * (symmetricShapeOperator(0,0) + symmetricShapeOperator(1,1) - sqrtDiscrim);
        gaussCurvatures[i] = princCurv1 * princCurv2;
        meanCurvatures[i] = 0.5 * (princCurv1 + princCurv2);
        */

        gaussCurvatures[i] = symmetricShapeOperator.determinant();
        meanCurvatures[i] = 0.5 * symmetricShapeOperator.trace();
    }
